...
public class graphics_panel extends JPanel{
	...
	public void draw_graphics(Graphics g){
		...
		bs2 = new BasicStroke(5, BasicStroke.CAP_SQUARE,BasicStroke.JOIN_MITER);g2d.setStroke(bs2);
		for (int i =1;i<=structure_file.counter_1;i++){
	...
	public int get_coord_upper(int start, int inc, int end, int limit, int x, int dir){int flag;for(int i = 0;i<Math.abs(start-limit);i++){flag=0;for (int j=x;j!=end;j=j+dir)if(grid[start][j]!=0&&grid[start][j+dir]!=0)flag=1;if (flag==0)return start;start=start+inc;}return 0;}
	public void line_vertical(int diff_1, int diff_2, int same, int direction){g2d.drawLine(same*10, diff_1*10, same*10, diff_2*10);if(direction==1){for (int k =diff_1;k<=diff_2;k=k+direction)grid[k][same]=1;}else{for (int k =diff_1;k>=diff_2;k=k+direction)grid[k][same]=1;}}
